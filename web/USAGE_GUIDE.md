...
访客进入只读论文展厅，只能查看管理员授予“默认未登录用户”的完成论文。访客不能访问内部项目状态、日志、文件、用户管理或控制动作。
...
5. 用户只看到 owner ACL 授权的可控制项目；页面下方的“授权展示论文”是只读集合，由公共访客集合和管理员为该用户单独授予的论文合并而成。

### 管理员

管理员可查看全部项目，直接创建项目，审批/拒绝用户和项目申请，按访客或具体注册用户配置完成论文展示权限，并查看审计日志与 Secret Manager 元数据健康状态。

系统没有默认管理员密码。不要从历史报告、测试脚本或命令历史获取登录值。

## 上传题目

“新建项目”支持上传或服务器路径两种方式。服务器路径只供管理员直接创建；普通用户的项目申请必须引用本次服务上传目录内的 PDF/Markdown，不能让后端读取任意服务器路径。
...
- 默认 8 阶段时间线，并可切换到 Step 0–16 高级视图、展开阶段内部步骤，同时查看当前 10 Stage/subtask/source Step 调度位置；
...
该命令只读取 Native `.factory/state.db`；无 Native 状态的历史项目不支持执行或自动推断。
...
新 `contest_core_v1` 项目会无条件在 Step 3 前等待 PRIMARY/AUXILIARY 选择；Web 会展示候选小样证据、主要风险和预计耗时，理由必填，并明确提示决策写入 SQLite 后不可覆盖。Web 与 CLI 是并行入口。
...
管理员面板会列出 `complete/` 中存在最终 PDF 的项目。选择“默认未登录用户”或某个注册用户、勾选论文并保存即可更新展示 ACL。注册用户始终继承访客公共集合，个人配置用于增加额外的只读论文。

展示 ACL 与 `project_acl` 相互独立：它只开放展厅中的最终 PDF，不开放项目状态、内部文件、日志、人工 gate 或暂停/恢复/终止动作。删除注册用户时，其个人展示授权也会一并删除。

`SHOWCASE_PROJECTS` 只在数据库首次升级到展示 ACL 时初始化访客集合；即使配置为空，之后也不会再次覆盖管理员保存的结果。

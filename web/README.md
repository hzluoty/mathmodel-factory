...
仓库提供 `apps/tui/`（Textual）作为本控制面的**只读**客户端。它不复制任何工作流逻辑——渲染的字段全部来自上面这些投影——因此终端与浏览器不会各自漂移出两套契约。
...
./run_tui.sh                 # 默认 http://127.0.0.1:8000
./run_tui.sh --base-url http://host:8000
# 等价入口： uv run --extra tui factory-tui ／ python -m apps.tui
```

登录屏的用户名与密码由你输入；`access_token` 仅驻内存，不落盘、不渲染。`ADMIN_PASSWORD` 仍只来自 Secret Manager。

| 位置 | 键 | 作用 |
| --- | --- | --- |
| 项目表 | `Enter` / `o` | 打开所选项目详情 |
| 项目表 | `r` | 重连实时通道（`/ws`） |
| 详情 | `l` | 在「阶段与产物」与「日志」之间切换 |
| 详情 | `r` | 立即刷新当前可见的投影 |
| 详情 | `Esc` / `b` | 返回项目表（过滤框打开时先关过滤框） |
| 日志 | `p` | 暂停／恢复跟随 |
| 日志 | `f` | 过滤（子串、大小写不敏感、边输边过滤） |
| 日志 | `End` | 跳到末尾并恢复跟随 |
| 任意 | `q` / `Ctrl+Q` | 退出 |

刷新节奏**有意保守**：状态靠 `/ws` 推送；`/steps` 与 `/diagnostics` 每 15s；日志每 5s，且只在日志面板可见、处于跟随状态、并且（项目在运行或尚未取过日志）时才请求。日志端点只读取文件尾部并在工作线程中执行，但终端仍然不去轮询没人看的面板。

能力限于只读：暂停、恢复、终止以及人工决策仍在浏览器或 CLI 中处理。
...
新 `contest_core_v1` 项目会无条件在 Step 3 前等待选择；可以在 Web 中提交，也可以在仓库根目录运行：
...
Web 使用 FactoryService 读取和控制 Native Stage 项目。实验快照页及 Phase 6/7/8
API 已拆至 `~/paper_new`，旧环境 flag 不再注册路由或加载实验数据库。
详见 [Native 主流程](../docs/architecture/NATIVE_MAINLINE.md)。

...
def _read_role_asset(manifest_dir: Path | None, relative: str) -> bytes:
    ...
    if (not isinstance(relative, str) or len(relative.split("/")) != 2
        ...
        raise ValueError("asset must be directly inside its role assets directory")
    # Resolve only after checking the original path. Otherwise a linked role
    # directory becomes its own apparent trusted root.
    for component in (manifest_dir, *manifest_dir.parents):
        if component.is_symlink():
            raise ValueError("role packet directory traverses a symlink")

...
def _render_context(
    ...
    assets = _asset_supplement(project, role)
    paths = sorted(
        paths,
...
def packet_payloads(
    ...
    for role, paths in selected.items():
        ...
        if assets:
            known = {_relative(project, path) for path in paths}
            paths.extend(project / relative for relative in assets if relative not in known)
            requirements.append({"id": "complete_role_local_assets",
                                 "description": "complete SHA-bound role-local source assets",
                                 "required_status": "included", "paths": list(assets)})

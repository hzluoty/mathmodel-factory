...
After artifact validation, the runner executes the cacheable `model` audit
profile. It validates the model/scope artifacts, `claim_registry.json`, and the
quality-contract schema, writes the snapshot under
`.factory/audits/profiles/model/`, and updates machine-owned rows in
...
missing invariants, model/implementation drift, and failed executable quality
claims before paper drafting begins.
...
Step 6 reruns the `results` profile with the sensitivity artifacts included in
the snapshot. Unchanged Step-5 inputs are reused; a changed result tree forces
fresh provenance, invariant, specification, and quality-contract checks.
...
rather than silently promoted to `CURRENT_PASS`.
...
- `judge_evaluation.md` with one of:
  - `VERDICT: PRECHECK_PASS` — continue to Step 14, but do not interpret this as
    final Gate-2 PASS;
  - `VERDICT: REOPEN_REVISION_MODEL` — substantive math failure; reopen through
...
Runner allows one scientific repair cycle. Judge infrastructure/grounding retries use a separate local retry path and do not consume that cycle. A repeated scientific reopen verdict blocks delivery instead of being converted into PASS. The final-submission judge may reuse a PASS only when the exact submission-input fingerprint is unchanged.
